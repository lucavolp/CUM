...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $dats = $data["data"];
    $usr_pers = implode(",", $data["selectedUsers"]);

    if (isset($data['dettagli']) && isset($data['data'])) {

        $sql = "INSERT INTO `Assenza` (`dettagli`, `data`, `utente_usr`) VALUES ('$dettagli', STR_TO_DATE('$dats', '%d/%m/%Y'), '$usr_pers')";
